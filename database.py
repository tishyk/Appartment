...
class Database():
    ...
    @property
    def current_mont_entry(self):
        """Get current month data entry from Database Table. None if not exists"""
        return self.get_entry(self.current_date.year, self.current_date.month)

    def get_entry(self, month, year):
        """Get data entry from Database Table"""
        query = 'SELECT {2} FROM {0} WHERE year={1};'.format(self.table_name, year, month)
        self.cursor.execute(query)
